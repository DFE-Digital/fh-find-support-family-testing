describe('FHG-924-FX-filter-cost.spec - Search by filter - Cost',function(){
    it('AC 1,3 Cost filter - No filter + Free filer',function(){
        ...
        // apply free filter    
        cy.costFilter('Free')
        cy.get('.govuk-grid-column-two-thirds').contains('£2.50 every session').should('not.exist')
        cy.get('.govuk-grid-column-two-thirds').contains('Free').should('exist')
        cy.get('.moj-filter__selected').contains('Free')
    });
    it('AC 2 Cost filter - Free',function(){
        cy.visit('/PostcodeSearch')
        cy.searchHubsPage()
        cy.searchbypostcode('M6 5UJ')
        cy.ServiceFilterPage();
